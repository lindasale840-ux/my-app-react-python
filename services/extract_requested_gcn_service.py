...
def run_requested_gcn_extractor_pure_simple(pdf_total_path, requested_gcn_text, process_ocr_func=None):
    ...
    # 2. THUẬT TOÁN FAST SKIP SCAN (QUÉT NHẢY CÓC THEO THIẾT KẾ ĐẦU TIÊN)
    ...
    for idx in pages_to_scan:
        # Truyền chính requested_list để lọc nhanh mã yêu cầu
        res = process_ocr_func(idx, doc[idx], requested_list, 300, 0.6)
        if res['gcn']:
            detected_map[idx] = res['gcn']
            
    # Quét lùi để xác định chính xác điểm biên xuất hiện đầu tiên của mã GCN
    ...
    for idx, gcn in list(detected_map.items()):
        ...
        for reverse_idx in range(idx, start_check - 1, -1):
            ...
            if res['gcn'] == gcn:
                confirmed_milestones[gcn] = reverse_idx # Cập nhật trang bắt đầu thật sự
            else:
                break
